...
class ChatAdapter (contexto : Context, chatLista : List<Chat>, imagenUrl : String)
    ...
    override fun onCreateViewHolder(parent: ViewGroup, position: Int): ViewHolder {
        return if (position == 1){
            val view : View = LayoutInflater.from(contexto).inflate(com.redsystem.proyectochatapp_kotlin.R.layout.message_right, parent, false)
            ViewHolder(view)
        }else{
            val view : View = LayoutInflater.from(contexto).inflate(com.redsystem.proyectochatapp_kotlin.R.layout.message_left,parent, false)
            ViewHolder(view)
        }
    }
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if (chat.getMensaje().equals("Изображение отправлено") && !chat.getUrl().equals("")){
            ...
            if (chat.getEmisor().equals(firebaseUser!!.uid)){
                ...
                holder.imagen_enviada_derecha!!.setOnClickListener {
                    val opciones = arrayOf<CharSequence>("Посмотреть полное изображение","Удалить изображение", "Отмена")
            ...
            /*Condición para el usuario el cuál nos envia una imagen como mensaje*/
                ...
                holder.imagen_enviada_izquierdo!!.setOnClickListener {
                    val opciones = arrayOf<CharSequence>("Посмотреть полное изображение", "Удалить")
                    val builder : AlertDialog.Builder = AlertDialog.Builder(holder.itemView.context)
                    builder.setTitle("Что вы хотите выполнить?")
                    builder.setItems(opciones, DialogInterface.OnClickListener{
                            dialogInterface, i ->
        ...
        /*Si el mensaje contiene sólo texto*/
            ...
            if (firebaseUser!!.uid == chat.getEmisor()){
                holder.TXT_ver_mensaje!!.setOnClickListener {
                    val opciones = arrayOf<CharSequence>("Удалить сообщение", "Отмена")
                    val builder : AlertDialog.Builder = AlertDialog.Builder(holder.itemView.context)
                    builder.setTitle("Что вы хотите выполнить?")
                    builder.setItems(opciones, DialogInterface.OnClickListener {
                            dialogInterface, i ->
        ...
        if (position == chatLista.size-1){
            if (chat.isVisto()){
                holder.TXT_mensaje_visto!!.text = "Отправлено"
                if (chat.getMensaje().equals("Изображение отправлено") && !chat.getUrl().equals("")){
                ...
                holder.TXT_mensaje_visto!!.text = "Получено"
                if (chat.getMensaje().equals("Изображение отправлено") && !chat.getUrl().equals("")){
                    val lp : RelativeLayout.LayoutParams = holder.TXT_mensaje_visto!!.layoutParams as LayoutParams
                    lp!!.setMargins(0,245, 10,0)
                    holder.TXT_mensaje_visto!!.layoutParams = lp
    ...
    private fun VisualizarImagen(imagen : String?){
        ...
        dialog.setContentView(R.layout.window_show_image)
    ...
    private fun EliminarMensaje(position: Int, holder : ChatAdapter.ViewHolder){
        ...
        val reference = FirebaseDatabase.getInstance().reference.child("Chats")
            ...
            .addOnCompleteListener { tarea->
                if (tarea.isSuccessful){
                    ...
                    Toast.makeText(holder.itemView.context, "Сообщение не было удалено, попробуйте еще раз", Toast.LENGTH_SHORT).show()
                }
            }
