...
import com.redsystem.proyectochatapp_kotlin.Model.ChatList
import com.redsystem.proyectochatapp_kotlin.Model.User
import com.redsystem.proyectochatapp_kotlin.Notifications.Token
...
class ChatFragment : Fragment() {
    ...
    private var usuarioListaChats : List<ChatList>?= null
    ...
    private fun RecuperarListaChats(){
        ...
        reference!!.addValueEventListener(object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                (userLista as ArrayList).clear()
                for (dataSnapshot in snapshot.children){
                    val user = dataSnapshot.getValue(User::class.java)
                    for (cadaLista in usuarioListaChats!!){
                        if (user!!.getUid().equals(cadaLista.getUid())){
                            (userLista as ArrayList).add(user!!)
                        }
                    }
                    usuarioAdaptador = UserAdapter(context!!, (userLista as ArrayList<User>), true)
